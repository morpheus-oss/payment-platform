...
public final class CloudEventFactory
{
	...
	public static CloudEvent create(String source, String type, byte[] payload)
	{
		...
		return CloudEventBuilder.v1().withId(UUID.randomUUID().toString()).withType(type).withSource(URI.create(source))
				.withTime(OffsetDateTime.now()).withData("application/json", payload).build();
	}
}
